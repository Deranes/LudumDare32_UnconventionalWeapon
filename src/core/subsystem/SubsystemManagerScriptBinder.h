...
static void BindSubsystemManagerToLua( )
{
	...
	DEV(
			...
			g_Script.Register( "GE_SetSimulationFramerate", [](IScriptEngine* scriptEngine) -> int { g_SubsystemManager.SetSimulationFramerate(scriptEngine->PopUint());	return 0; } );
	);
	g_Script.Register( "GE_GetSimulationFramerate", [](IScriptEngine* scriptEngine) -> int { scriptEngine->PushUint(g_SubsystemManager.GetSimulationFramerate());	return 1; } );
}
